...
We set lower penalty for early terms (powers 1-3) and higher for the rest.
...
We use `alpha=0` for Ridge regression (L2) to handle multicollinearity common in high-order polynomials.

```{r}
fit <- cv.glmnet(X_poly, y, penalty.factor = weights, alpha = 0)
```
